...
namespace det {
static DD4hep::Geometry::Ref_t createBarrelPhiZLayer(DD4hep::Geometry::LCDD& aLcdd,
  ...
  DD4hep::XML::Dimension  layer_dim(layer_det.dimensions());
  ...
  uint numLayer = cell_dim.dz() / layer_dim.dz();
  std::cout << " Number of layers in cell: " << numLayer << ", cells in phi: " << numPhi <<" and in Z: " << numZ << " = total of " << numLayer * numPhi * numZ << std::endl;

  // create cell volume
  DD4hep::Geometry::Volume cell_vol("cell_vol", DD4hep::Geometry::Trapezoid(
      dXmin / 2., dXmax / 2., cell_dim.dz(), cell_dim.dz(), dR / 2.
      ), aLcdd.material("Air") );
  // create sensitive layer volume
  double offsetLayer = - cell_dim.dz() + layer_dim.dz();
  DD4hep::Geometry::Volume layer_vol("layer_vol", DD4hep::Geometry::Trapezoid(
      dXmin / 2., dXmax / 2., layer_dim.dz(), layer_dim.dz(), dR / 2.
      ), layer_mat );
  // mark as sensitive
  ...
  for ( uint iLayer = 0; iLayer < numLayer; iLayer++ ) {
    std::cout << " Placing layer " << iLayer <<" inside cell at y = " << offsetLayer + iLayer * 2. * layer_dim.dz() << std::endl; // y before rotation, afterwards it becomes z
    DD4hep::Geometry::Transform3D transformlayer(
      DD4hep::Geometry::Rotation3D(DD4hep::Geometry::RotationZYX(0,0,0)),
      DD4hep::Geometry::Position(0,offsetLayer+iLayer*2.*layer_dim.dz(),0 ));
    DD4hep::Geometry::PlacedVolume layer_physvol = cell_vol.placeVolume(layer_vol,transformlayer);
  ...
  DD4hep::Geometry::Volume slice_vol("slice_vol",
    ...
    aLcdd.material("Air"));
  // place cells (consisting of layers) into slices, rotated by phi angle (dPhi)
  double offsetPhi = 0;
...
DECLARE_DETELEMENT(BarrelPhiZLayer, det::createBarrelPhiZLayer)
