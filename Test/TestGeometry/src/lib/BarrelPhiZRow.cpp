...
namespace det {
static DD4hep::Geometry::Ref_t createBarrelPhiZRow(DD4hep::Geometry::LCDD& aLcdd,
  ...
  DD4hep::Geometry::Material row_mat (aLcdd.material(row_det.materialStr()));
  std::cout << "material: " << row_det.materialStr() << std::endl;
  ...
  std::cout << " Number of rows in cell: " << numRow << ", cells in phi: " << numPhi <<" and in Z: " << numZ << " = total of " << numRow * numPhi * numZ << std::endl;

  // create cell volume
  DD4hep::Geometry::Volume cell_vol("cell_vol", DD4hep::Geometry::Trapezoid(
      dXmin / 2., dXmax / 2., cell_dim.dz(), cell_dim.dz(), dR / 2.
      ), aLcdd.material("Air") );
  // create sensitive row volume
  ...
  row_vol.setSensitiveDetector(aSensDet);
...
DECLARE_DETELEMENT(BarrelPhiZRow, det::createBarrelPhiZRow)
